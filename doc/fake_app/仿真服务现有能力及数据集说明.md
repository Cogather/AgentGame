...
| 构造完整评测集（调接口补 houses） | AgentGame/fake_app/py/build_eval_dataset.py；默认输入 case_init.json、输出 case_result.json；支持 --case-ids / --case-id-range 仅处理指定案例 |
| Agent 可集成的仿真服务工具集说明（OpenAPI 3） | AgentGame/fake_app/data/fake_app_agent_tools.json |
| house/landmark 代码解读 | AgentGame/doc/fake_app/house_go_解读.md、landmark_go_解读.md |
